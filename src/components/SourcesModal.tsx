...
import { X, Search, ExternalLink, Globe, Layers, Filter, CheckCircle2, Copy, Check } from "lucide-react";
...
export const SourcesModal: React.FC<SourcesModalProps> = ({
  ...
  const handleCopyEndpoint = (id: number, endpointUrl: string) => {
      ...
      <div className="bg-slate-900 border border-blue-800/80 rounded-2xl w-full max-w-4xl max-h-[85vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="p-4 border-b border-slate-800 bg-slate-900/90 flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Buscar por ID, categoria ou endpoint da API..."
              value={searchTerm}
        ...
        <div className="p-5 overflow-y-auto flex-1 custom-scrollbar">
          <div className="text-xs text-slate-400 mb-3 flex items-center justify-between">
            <span>
              Mostrando <strong className="text-blue-300">{filteredSources.length}</strong> endpoints integrados
            </span>
            <span className="flex items-center gap-1 text-emerald-400">
              <CheckCircle2 className="w-3.5 h-3.5" /> Endpoints de API ativos e sincronizados
            </span>
          </div>
